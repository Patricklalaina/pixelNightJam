...
func _ready():
	...
	progress.value = timer.wait_time
	timer.one_shot = true
	visible = false  # démarrer caché

func start_timer():
	if not timer.is_stopped():
		timer.stop()  # Arrêter le timer s'il était déjà en cours

	# S'assurer que le conteneur et la barre sont visibles à chaque démarrage
	visible = true
	progress.visible = true

	# (Re)initialiser les bornes au cas où wait_time change
	progress.min_value = 0
	progress.max_value = timer.wait_time
	progress.value = timer.wait_time
